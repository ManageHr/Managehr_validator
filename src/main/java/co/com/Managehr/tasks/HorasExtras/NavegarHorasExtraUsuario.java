...
public class NavegarHorasExtraUsuario implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        try {
            actor.attemptsTo(
                    WaitUntil.the(FormHorasExtra.MENU_HORAS_EXTRA, isVisible())
                            .forNoMoreThan(30).seconds(),
                    Click.on(FormHorasExtra.MENU_HORAS_EXTRA),
                    WaitUntil.the(FormHorasExtra.FORM_CONTENEDOR, isVisible())
                            .forNoMoreThan(30).seconds()
            );
            Thread.sleep(3000);
            ...
            System.out.println("Error no encontró el menú de Horas Extra (vista usuario)");
        }
    }
}
